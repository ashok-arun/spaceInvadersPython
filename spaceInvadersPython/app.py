...
class Invaders(GameApp):
    ...
    #
    # Attribute _level: the currently active message
    # Invariant: _level is a GLabel object, or None if there is no message to
    # display. It is onl None if _state is STATE_ACTIVE.
    #
    # Attribute _life: the currently active message
    # Invariant: _life is a GLabel object, or None if there is no message to
    # display. It is onl None if _state is STATE_ACTIVE.
    #
    ...
    def start(self):
        ...
        self._level=None
        self._life=None
        self._text=GLabel(text="Space Invaders\nAshok Arun and " +
                         "Christian Bioswell\nPress 'SPACE' to Play",
                          font_name='Arcade.ttf', x=GAME_WIDTH//2,
                          y=GAME_HEIGHT//2, font_size=40)
        self._pause=False
    ...
    def update(self,dt):
        ...
        self.updateCONTINUE()
        self.updateCOMPLETE()
        self._pause=self.input.is_key_down('s')
    ...
    def updateNEWWAVE(self):
        ...
        if self._state==STATE_NEWWAVE:
                if self._wave==None:
                    ...
                    self._wave=Wave(level=self._wave.getLevel()+1,
                                    lives=self._wave.getLives(),
                                    score=self._wave.getScore())
                self._score=GLabel(text="Score\n"+
                                          str(self._wave.getScore()),
                                          font_name='Arcade.ttf',
                                          x=GAME_WIDTH/2, y=GAME_HEIGHT-50,
                                          font_size=30)
                self._level=GLabel(text="Level\n"+
                                          str(self._wave.getLevel()),
                                          font_name='Arcade.ttf',
                                          x=GAME_WIDTH/4, y=GAME_HEIGHT-50,
                                          font_size=30)
                self._life=GLabel(text="Lives\n"+
                                         str(self._wave.getLives()),
                                         font_name='Arcade.ttf',
                                         x=3*(GAME_WIDTH/4), y=GAME_HEIGHT-50,
                                         font_size=30)
    ...
    def updateACTIVE(self, dt):
        ...
        if self._state==STATE_ACTIVE:
            ...
            self._score=GLabel(text="Score\n"+str(self._wave.getScore()),
            font_name='Arcade.ttf', x=GAME_WIDTH/2, y=GAME_HEIGHT-50, font_size=30)
            self._level=GLabel(text="Level\n"+str(self._wave.getLevel()),
            font_name='Arcade.ttf', x=GAME_WIDTH/4, y=GAME_HEIGHT-50, font_size=30)
            self._life=GLabel(text="Lives\n"+str(self._wave.getLives()),
            font_name='Arcade.ttf', x=3*(GAME_WIDTH/4),y=GAME_HEIGHT-50, font_size=30)
            if self._wave.getShip()==None and self._wave.getLives()>0:
                self._state=STATE_PAUSED
                self._text=GLabel(text="You Died: "+str(self._wave.getLives())+
                " leves left\nPress 's' to Resume", font_name='Arcade.ttf',
                x=GAME_WIDTH//2,y=GAME_HEIGHT//2, font_size=40)
            elif self._wave.game_over():
                self._state=STATE_COMPLETE
                self._level=None
                self._life=None
                self._text=GLabel(text="You Lost\nPress 's' to restart the game",
                font_name='Arcade.ttf',x=GAME_WIDTH//2,y=GAME_HEIGHT//2,font_size=40)
            elif self._wave.won():
                self._state=STATE_COMPLETE
                self._text=GLabel(text="You Won\nPress 'SPACE' to go to level "+
                str(self._wave.getLevel()+1),font_name='Arcade.ttf', x=GAME_WIDTH//2,
                y=GAME_HEIGHT//2, font_size=40)
            elif self.input.is_key_down('s') and not self._pause:
                self._state=STATE_PAUSED
                self._text=GLabel(text="Game is paused\nPress 's' to resume",
                font_name='Arcade.ttf',x=GAME_WIDTH//2, y=GAME_HEIGHT//2,font_size=40)
                self._pause=True
    ...
    def updatePAUSED(self):
        ...
        """
        if self._state==STATE_PAUSED:
            if self.input.is_key_down('s') and not self._pause:
                self._text=None
                self._pause=True
                self._state=STATE_CONTINUE

    def updateCONTINUE(self):
        """
    ...
    def updateCOMPLETE(self):
        ...
        """
        if self._state==STATE_COMPLETE and self._wave.won():
            if self.input.is_key_down('spacebar'):
                self._state=STATE_NEWWAVE
                self._text=None
        if self._state==STATE_COMPLETE and self._wave.game_over():
            if self.input.is_key_down('s') and not self._pause:
                self._state=STATE_INACTIVE
                self._wave=None
                self._score=None
